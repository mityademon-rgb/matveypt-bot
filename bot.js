...
bot.onText(/\/start/, async (msg) => {
  ...
  const greeting = `Привет! Я Матвей 👋

Работаю продюсером на Первом Туристическом — федеральном канале про путешествия. Моя работа — помогать рекламодателям запускать кампании: от идеи до эфира.

Говорю по-человечески, без воды. Если что-то непонятно — сразу скажу и объясню.

Через кнопки внизу можно посмотреть всё о канале, форматах размещения или прикинуть бюджет. А пока давайте познакомимся!
